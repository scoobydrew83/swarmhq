...
const SUBCOMMANDS: Record<string, string[]> = {
  ...
  nodes: ["promote", "demote", "label"],
  service: ["inspect", "tasks"],
  stack: ["deploy", "ls", "ps", "services", "rm"],
  secret: ["create", "ls", "inspect", "rm"],
  configs: ["create", "ls", "inspect", "rm"],
  network: ["ls", "inspect", "create", "rm"],
...
const FLAGS: Record<string, string[]> = {
  ...
  nodes: ["--config", "--context", "--target", "--key", "--value", "--json", "--yes"],
  services: ["--config", "--context", "--json"],
  service: ["--config", "--context", "--name", "--all", "--json"],
  stack: ["--config", "--context", "--file", "--name", "--json", "--yes"],
  secret: ["--config", "--context", "--name", "--file", "--stdin", "--json", "--yes"],
  configs: ["--config", "--context", "--name", "--file", "--stdin", "--json", "--yes"],
  network: ["--config", "--context", "--name", "--driver", "--no-attachable", "--json", "--yes"],
  logs: ["--config", "--context", "--name", "--follow", "--since", "--tail"],
